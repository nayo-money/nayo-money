...
type Tab = "overview" | "home" | "menu" | "posts" | "categories" | "crystals" | "products";
...
function HomeEditor({ notify, fail }: { notify:(s:string)=>void; fail:(s:string)=>void }) {
  ...
  return <section className="editor">
    ...
    <div className="form-grid">
      ...
      <div className="form-section-title full">網站主選單</div>
      <Field label="首頁名稱" value={form.nav_home_label} onChange={v=>set("nav_home_label",v)}/>
      <Field label="首頁連結" value={form.nav_home_url} onChange={v=>set("nav_home_url",v)}/>
      <Field label="Blog 主選單名稱" value={form.nav_blog_label} onChange={v=>set("nav_blog_label",v)}/>
      <Field label="Blog 主選單連結" value={form.nav_blog_url} onChange={v=>set("nav_blog_url",v)}/>
      <Field label="Blog－全部文章名稱" value={form.nav_blog_all_label} onChange={v=>set("nav_blog_all_label",v)}/>
      <Field label="Blog－全部文章連結" value={form.nav_blog_all_url} onChange={v=>set("nav_blog_all_url",v)}/>
      <Field label="Blog－信用卡回饋名稱" value={form.nav_blog_credit_label} onChange={v=>set("nav_blog_credit_label",v)}/>
      <Field label="Blog－信用卡回饋連結" value={form.nav_blog_credit_url} onChange={v=>set("nav_blog_credit_url",v)}/>
      <Field label="Blog－小資理財名稱" value={form.nav_blog_finance_label} onChange={v=>set("nav_blog_finance_label",v)}/>
      <Field label="Blog－小資理財連結" value={form.nav_blog_finance_url} onChange={v=>set("nav_blog_finance_url",v)}/>
      <Field label="Blog－旅行 × 生活名稱" value={form.nav_blog_lifestyle_label} onChange={v=>set("nav_blog_lifestyle_label",v)}/>
      <Field label="Blog－旅行 × 生活連結" value={form.nav_blog_lifestyle_url} onChange={v=>set("nav_blog_lifestyle_url",v)}/>
      <Field label="Blog－生命靈數 × 水晶名稱" value={form.nav_blog_crystal_label} onChange={v=>set("nav_blog_crystal_label",v)}/>
      <Field label="Blog－生命靈數 × 水晶連結" value={form.nav_blog_crystal_url} onChange={v=>set("nav_blog_crystal_url",v)}/>
      <Field label="Crystal 主選單名稱" value={form.nav_crystal_label} onChange={v=>set("nav_crystal_label",v)}/>
      <Field label="Crystal 主選單連結" value={form.nav_crystal_url} onChange={v=>set("nav_crystal_url",v)}/>
      <Field label="Crystal－生命靈數名稱" value={form.nav_crystal_life_label} onChange={v=>set("nav_crystal_life_label",v)}/>
      <Field label="Crystal－生命靈數連結" value={form.nav_crystal_life_url} onChange={v=>set("nav_crystal_life_url",v)}/>
      <Field label="Crystal－缺數水晶名稱" value={form.nav_crystal_missing_label} onChange={v=>set("nav_crystal_missing_label",v)}/>
      <Field label="Crystal－缺數水晶連結" value={form.nav_crystal_missing_url} onChange={v=>set("nav_crystal_missing_url",v)}/>
      <Field label="Crystal－缺數手環作品名稱" value={form.nav_crystal_work_label} onChange={v=>set("nav_crystal_work_label",v)}/>
      <Field label="Crystal－缺數手環作品連結" value={form.nav_crystal_work_url} onChange={v=>set("nav_crystal_work_url",v)}/>
      <Field label="Crystal－購買須知名稱" value={form.nav_crystal_buy_label} onChange={v=>set("nav_crystal_buy_label",v)}/>
      <Field label="Crystal－購買須知連結" value={form.nav_crystal_buy_url} onChange={v=>set("nav_crystal_buy_url",v)}/>
      <Field label="關於主選單名稱" value={form.nav_about_label} onChange={v=>set("nav_about_label",v)}/>
      <Field label="關於主選單連結" value={form.nav_about_url} onChange={v=>set("nav_about_url",v)}/>

      <div className="form-section-title full">頁尾</div>
      <Field label="頁尾品牌說明" value={form.footer_tagline} onChange={v=>set("footer_tagline",v)} full/>
      <Field label="頁尾「網站導覽」標題" value={form.footer_blog_title} onChange={v=>set("footer_blog_title",v)}/>
      <Field label="頁尾「Crystal」標題" value={form.footer_crystal_title} onChange={v=>set("footer_crystal_title",v)}/>
      <Field label="頁尾「關於」標題" value={form.footer_about_title} onChange={v=>set("footer_about_title",v)}/>
      <Field label="頁尾管理後台名稱" value={form.footer_admin_label} onChange={v=>set("footer_admin_label",v)}/>
      <Field label="頁尾管理後台連結" value={form.footer_admin_url} onChange={v=>set("footer_admin_url",v)}/>
      <Field label="頁尾版權文字" value={form.footer_copyright} onChange={v=>set("footer_copyright",v)} full/>
...
function MenuEditor({notify,fail}:{notify:(s:string)=>void;fail:(s:string)=>void}){
  ...
  const addChild=(parentId:string)=>{
    setItems(prev=>prev.map(x=>x.id===parentId?{...x,children:[...(x.children||[]),{id:`menu-${crypto.randomUUID()}`,label:"新下拉項目",url:"/"}]}:x));
  };
  const addCategory=()=>{
    ...
    setItems(prev=>prev.map(x=>{
      if(x.id!==categoryParent)return x;
      const children=x.children||[];
      if(children.some(y=>y.url===child.url || y.id===child.id))return x;
      return {...x,children:[...children,child]};
    }));
    setCategoryId("");
  };
  const save=async()=>{
    ...
    try{
      const {error}=await adminSupabase.from("site_settings").upsert({
        setting_key:"menu_json",
        setting_value:JSON.stringify(items),
        updated_at:new Date().toISOString()
      },{onConflict:"setting_key"});
      if(error)throw error;
      notify("網站選單已儲存。");
    }catch(e:any){fail(e.message||"網站選單儲存失敗")}
  ...
  return <section className="editor">
    <div className="editor-head">
      <div><p className="eyebrow">MENU CMS</p><h2>網站選單</h2><p>上方主選單與下拉選項都可以新增、修改、刪除，不再寫死在程式裡。</p></div>
      <button className="primary compact" onClick={save} disabled={saving}>{saving?"儲存中…":"儲存網站選單"}</button>
    </div>

    <div className="menu-category-box">
      <strong>把「文章分類」加入下拉選單</strong>
      <p>先到「文章分類」建立分類，再在這裡選擇要放入哪個主選單。</p>
      <div className="form-grid">
        <label>放入主選單
