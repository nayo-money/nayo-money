...
function PromotionsEditor({notify,fail}:{notify:(s:string)=>void;fail:(s:string)=>void}){
  ...
  useEffect(()=>{load()},[]);
  async function save(row:Row){if(!supabase)return;const payload: Record<string, any> = {...row,bullets:String(row.bullets||"").split("\n").map((x:string)=>x.trim()).filter(Boolean),meta:String(row.meta||"").split("\n").map((x:string)=>x.trim()).filter(Boolean),sort_order:Number(row.sort_order||0)};delete payload.id;delete payload.created_at;delete payload.updated_at;const result=row.id?await adminSupabase.from("promotions").update(payload).eq("id",row.id):await adminSupabase.from("promotions").insert(payload);if(result.error)return fail(result.error.message);notify(row.id?"信用卡優惠已更新。":"信用卡優惠已新增。");setEditing(null);load()}
