...
export default function AdminPage() {
  ...
  async function checkAdmin(currentUser: User) {
    ...
    const fallback = await adminSupabase.from("admins").select("id").eq("id", currentUser.id).maybeSingle();
...
function HomeEditor({ notify, fail }: { notify:(s:string)=>void; fail:(s:string)=>void }) {
  const [form, setForm] = useState<Row>({ site_name:"Nayo 娜攸", site_tagline:"生活理財 × 水晶", profile_name:"娜攸", profile_image:"", hero_image:"", hero_title:"把理財變成，喜歡的生活。", hero_highlight:"喜歡的生活", hero_description:"分享信用卡回饋、小資理財、旅行生活，也記錄生命靈數與水晶。", primary_label:"逛理財 Blog", primary_url:"/blog", secondary_label:"探索 Nayo Crystal", secondary_url:"/crystal", about_text:"理財讓生活有更多選擇，而生命靈數與水晶，是我探索生活與自己的另一種方式。" });
  const [loading,setLoading]=useState(true); const [saving,setSaving]=useState(false);
  useEffect(()=>{(async()=>{if(!supabase)return;const {data,error}=await adminSupabase.from("site_settings").select("*").eq("id",true).maybeSingle();if(error)fail(error.message);else if(data)setForm(data);setLoading(false)})()},[fail]);
  const set=(k:string,v:any)=>setForm((x:Row)=>({...x,[k]:v}));
  async function image(field:string,file?:File){if(!file)return;try{const url=await uploadImage(file,"home");set(field,url);notify("圖片已上傳，記得按儲存首頁設定。")}catch(e:any){fail(e.message||"圖片上傳失敗")}}
  async function save(){if(!supabase)return;setSaving(true);const {error}=await adminSupabase.from("site_settings").upsert({...form,id:true,updated_at:new Date().toISOString()});if(error)fail(error.message);else notify("首頁設定已儲存。");setSaving(false)}
  if(loading)return <section className="empty-card"><p>載入首頁設定…</p></section>;
  return <section className="editor"><div className="editor-head"><div><p className="eyebrow">HOME CMS</p><h2>首頁內容</h2><p>換圖、改標題、改按鈕，不需要再修改程式。</p></div><button className="primary compact" onClick={save} disabled={saving}>{saving?"儲存中…":"儲存首頁設定"}</button></div>
    <div className="form-grid"><Field label="網站名稱" value={form.site_name} onChange={v=>set("site_name",v)}/><Field label="網站副標" value={form.site_tagline} onChange={v=>set("site_tagline",v)}/><Field label="個人名稱" value={form.profile_name} onChange={v=>set("profile_name",v)}/><TextArea label="關於我" value={form.about_text} onChange={v=>set("about_text",v)} full/>
      <Field label="主標題" value={form.hero_title} onChange={v=>set("hero_title",v)} full/><Field label="主標題強調文字" value={form.hero_highlight} onChange={v=>set("hero_highlight",v)}/><Field label="首頁說明" value={form.hero_description} onChange={v=>set("hero_description",v)} full/>
      <Field label="第一顆按鈕文字" value={form.primary_label} onChange={v=>set("primary_label",v)}/><Field label="第一顆按鈕連結" value={form.primary_url} onChange={v=>set("primary_url",v)}/><Field label="第二顆按鈕文字" value={form.secondary_label} onChange={v=>set("secondary_label",v)}/><Field label="第二顆按鈕連結" value={form.secondary_url} onChange={v=>set("secondary_url",v)}/>
      <ImageField label="大頭貼" value={form.profile_image} onChange={file=>image("profile_image",file)} setUrl={v=>set("profile_image",v)}/><ImageField label="首頁主圖" value={form.hero_image} onChange={file=>image("hero_image",file)} setUrl={v=>set("hero_image",v)}/>
    </div></section>;
}
