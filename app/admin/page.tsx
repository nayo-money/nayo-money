...
function PostForm({row,cats,onSave,onCancel,onUpload}:{row:Row;cats:Row[];onSave:(r:Row)=>void;onCancel:()=>void;onUpload:(f:File)=>Promise<void>}){const [x,setX]=useState(row);const set=(k:string,v:any)=>setX({...x,[k]:v});return <section className="editor"><EditorHead title={x.id?"編輯文章":"新增文章"} onSave={()=>onSave(x)} onCancel={onCancel}/><div className="form-grid"><Field label="標題" value={x.title} onChange={v=>set("title",v)} full/><Field label="Slug" value={x.slug} onChange={v=>set("slug",v)}/><select className="field" value={x.category_id||""} onChange={e=>set("category_id",e.target.value)}><option value="">未分類</option>{cats.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><select className="field" value={x.status} onChange={e=>set("status",e.target.value)}><option value="draft">草稿</option><option value="published">發布</option></select><Field label="SEO 標題" value={x.seo_title||""} onChange={v=>set("seo_title",v)} /><Field label="SEO 描述" value={x.seo_description||""} onChange={v=>set("seo_description",v)} full/><TextArea label="摘要" value={x.excerpt||""} onChange={v=>set("excerpt",v)} full/><RichTextEditor label="文章內容" value={x.content||""} onChange={v=>set("content",v)}/><ImageField label="封面圖片" value={x.cover_image||""} onChange={onUpload} setUrl={v=>set("cover_image",v)}/></div></section>}

function RichTextEditor({label,value,onChange}:{label:string;value:string;onChange:(v:string)=>void}){const ref=useRef<HTMLDivElement|null>(null);const [source,setSource]=useState(false);useEffect(()=>{if(ref.current && ref.current.innerHTML!==value) ref.current.innerHTML=value||""},[value]);const command=(cmd:string,arg?:string)=>{ref.current?.focus();document.execCommand(cmd,false,arg);onChange(ref.current?.innerHTML||"")};const block=(tag:string)=>{ref.current?.focus();document.execCommand("formatBlock",false,tag);onChange(ref.current?.innerHTML||"")};const link=()=>{const url=window.prompt("請輸入連結網址");if(url) command("createLink",url)};if(source)return <label className="field-label full"><span>{label}（HTML 原始碼）</span><textarea rows={14} value={value||""} onChange={e=>onChange(e.target.value)}/><button type="button" className="soft-btn" style={{marginTop:8}} onClick={()=>setSource(false)}>返回編輯器</button></label>;return <div className="field-label full rich-editor-field"><span>{label}</span><div className="rich-toolbar"><button type="button" onClick={()=>command("bold")}><b>B</b></button><button type="button" onClick={()=>command("italic")}><i>I</i></button><button type="button" onClick={()=>command("underline")}><u>U</u></button><button type="button" onClick={()=>block("h2")}>H2</button><button type="button" onClick={()=>block("h3")}>H3</button><button type="button" onClick={()=>command("insertUnorderedList")}>• 清單</button><button type="button" onClick={()=>command("insertOrderedList")}>1. 清單</button><button type="button" onClick={link}>🔗 連結</button><button type="button" onClick={()=>command("removeFormat")}>清除格式</button><button type="button" className="source-btn" onClick={()=>setSource(true)}>HTML</button></div><div ref={ref} className="rich-editor" contentEditable suppressContentEditableWarning onInput={()=>onChange(ref.current?.innerHTML||"")} /> <small className="rich-help">直接像 Word 一樣輸入文章；需要時可用 H2／H3、條列與連結。封面圖會另外顯示在文章最上方。</small></div>}
