...
function RichTextEditor({value,onChange}:{value:string;onChange:(v:string)=>void}){
  ...
  function exec(command:string,arg?:string){
    ...
    setHtml(next); onChange(next);
  }
  function sync(){ const next=ref.current?.innerHTML||""; setHtml(next); onChange(next); }
  if(source) return <div className="rich-editor"><div className="rich-toolbar"><button type="button" onClick={()=>setSource(false)}>↔ 視覺編輯</button></div><textarea className="rich-source" value={html} onChange={e=>{setHtml(e.target.value);onChange(e.target.value)}} placeholder="HTML 原始碼（進階使用）"/></div>;
  return <div className="rich-editor">
    <div className="rich-toolbar">
      ...
      <button type="button" onClick={()=>exec("insertUnorderedList")}>• 條列</button>
      <button type="button" onClick={()=>exec("insertOrderedList")}>1. 編號</button>
      <button type="button" onClick={()=>exec("formatBlock","<blockquote>")}>❝ 引用</button>
      <button type="button" onClick={()=>{const url=window.prompt("請輸入連結網址");if(url)exec("createLink",url)}}>🔗 連結</button>
      <button type="button" onClick={()=>exec("removeFormat")}>清除格式</button>
      <button type="button" onClick={()=>setSource(true)}>HTML</button>
    </div>
