...
function SimpleCrud({table,title,fields,notify,fail,imageFolder}:{table:string;title:string;fields:{k:string;l:string;area?:boolean;number?:boolean}[];notify:(s:string)=>void;fail:(s:string)=>void;imageFolder?:string}){
  ...
  async function save(row:Row){if(!supabase)return;const payload={...row};if(table==="categories"){payload.type=payload.type||"blog";if(payload.is_active==null)payload.is_active=true;}if(table==="products"){payload.missing_numbers=payload.missing_numbers==null?"":String(payload.missing_numbers);payload.category=payload.category==null?"":String(payload.category);payload.status="published";}if(payload.id){const {error}=await adminSupabase.from(table).update(payload).eq("id",payload.id);if(error)return fail(error.message)}else{delete payload.id;const {error}=await adminSupabase.from(table).insert(payload);if(error)return fail(error.message)}notify(`${title}已儲存。`);setEditing(null);await load()}
