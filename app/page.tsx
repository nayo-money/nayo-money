    ...
    <section className="container section" id="card-promotions">
      ...
      <div className="promo-carousel">
        {displayPromotions.map((card) => <article className="promo-card" key={card.id}>
          ...
          <div className="promo-body"><span>🎁 新戶首刷好禮四選一</span><ul>{card.bullets.map((b: string) => <li key={b}>{b}</li>)}</ul></div>
          <div className="promo-meta">{card.meta.map((m: string) => <div key={m}>◷ {m}</div>)}<span className="promo-deadline">▣ {card.deadline}</span></div>
