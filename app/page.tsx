...
export default async function Home() {
  ...
  if (supabase) {
    const [s, p, pr, c, promo] = await Promise.all([
      ...
      supabase.from("promotions").select("id,bank,badge,title,subtitle,image,bullets,meta,deadline,url,sort_order").order("sort_order").order("created_at",{ascending:false}),
    ]);
    if (s.data) {
      settings = { ...fallback };
    ...
    <section className="container section">
      <div className="split">
        <div className="feature-box">
          ...
          <h3>{settings.footer_blog_title || "生活理財 Blog"}</h3>
          <p>{settings.footer_blog_description || "信用卡回饋、小資理財、ETF／投資、旅行 × 生活，把實用資訊整理成容易閱讀的筆記。"}</p>
          <Link className="more" href={settings.footer_blog_url || "/blog"}>{settings.footer_blog_button || "進入理財 Blog →"}</Link>
        </div>
        <div className="feature-box crystal">
          <div className="section-kicker">NAYO CRYSTAL</div>
          <h3>{settings.footer_crystal_title || "生命靈數 × 水晶"}</h3>
          <p>{settings.footer_crystal_description || "缺數解析、天然水晶、客製手環與作品展示。從生命靈數認識自己，再找到適合自己的水晶。"}</p>
          <Link className="more" href={settings.footer_crystal_url || "/crystal"}>{settings.footer_crystal_button || "探索 Crystal →"}</Link>
          <Link className="more" href={settings.footer_crystal_buy_url || "/crystal/buy"}>{settings.footer_crystal_buy_button || "購買須知 →"}</Link>
        </div>
      </div>
    </section>
