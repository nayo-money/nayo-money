...
export async function Header() {
  ...
  const fallback = { site_name: "Nayo 娜攸", site_tagline: "生活理財 × 水晶", site_logo_url: "/icon.svg" };
  let siteName = fallback.site_name;
  let siteTagline = fallback.site_tagline;
  let siteLogo = fallback.site_logo_url;
  ...
  if (supabase) {
    ...
    for (const row of data || []) {
      ...
      if (row.setting_key === "site_logo_url") siteLogo = row.setting_value || fallback.site_logo_url;
    }
  }
    ...
    <header className="site-header">
      <nav className="nav">
        <Link href="/" className="brand" aria-label={`${siteName}首頁`}>
          <span className="brand-mark">{siteLogo ? <img src={siteLogo} alt="" /> : "N"}</span>
